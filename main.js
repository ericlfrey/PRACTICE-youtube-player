...
const eventListeners = () => {
  ...
  document.querySelector('#filterContainer').addEventListener('click', (e) => {
    ...
    if (e.target.id === 'clear') {
      ...
      const favoriteFilter = data.filter(item => item.favorite);
      cardsOnDom(favoriteFilter);
    } else if (e.target.id) {
      const topics = data.filter(item => item.category === e.target.id);
      cardsOnDom(topics);
    }
  });
  ...
  document.querySelector('#cardContainer').addEventListener('click', (e) => {
    ...
    if (e.target.id) {
      ...
      const index = data.findIndex(item => item.videoId === id);
